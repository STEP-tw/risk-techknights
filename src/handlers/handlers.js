...
const loadContinents = function (TERRITORIES) {
  const CONTINENTS = {};
  const continents = JSON.parse(fs.readFileSync('./src/data/continent.json', ENCODING));
...
const createGame = function (req) {
  ...
  const continents = loadContinents(territories);
