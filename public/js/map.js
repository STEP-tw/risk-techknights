...
const displayTerritory = function (document) {
  ...
  for (const territoryPath in TERRITORIES_PATH) {
    const path = `<a id='${territoryPath}' onclick='changeColor()'> 
...
const addMilitaryUnit = function (territory) {
  ...
  militaryUnit.setAttribute('transform', 'translate(' + x + ' ' + (y + 13) + ')');
...
const changeColor = function () {
  ...
  if (i % 2 == 0) event.target.style.fill = "#F08080";
  else event.target.style.fill = "#98FB98";
  i++;
}
