...
const getAttackerDiceValue = function () {
  return ['attacker-dice1', 'attacker-dice2', 'attacker-dice3'].map(dice =>
    +getElementInnerText(document, dice)).sort().reverse();
}

const getDefenderDiceValue = function () {
  return ['defender-dice1', 'defender-dice2'].map(dice =>
    +getElementInnerText(document, dice)).sort().reverse();
}
...
const battleComplete = function () {
  fetch('/battleComplete', sendPostRequest({}))
    .then(res => res.json())
    .then(territory => {
      document.getElementById(territory.attackingTerritory).childNodes[1].style.fill = "#f2f2f2";
      document.getElementById(territory.defendingTerritory).childNodes[1].style.fill = "#f2f2f2";
    })
